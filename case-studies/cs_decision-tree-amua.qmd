...
-   Define the following three strategies
    1.  No screening
    2.  Screen
    3.  Treat all
...
-   The text will now turn [red]{style="color: red;"} because Amua does not recognize prev as a model object, so we will have to define it later.
...
-   Next, we'll define the probability of having the disease given each test result in the `Screen` strategy.
    ...
    4.  Click under **D-** below and type `C` again.
        ...
        ![](images/amua-prob3.png)

-   Finally, define the probability of having the disease in the `Treat All` strategy.

    1.  Click under **D+** and type `prev`, which is the prevalence of the disease.

    2.  Enter `C` for the probability of **D-**.
